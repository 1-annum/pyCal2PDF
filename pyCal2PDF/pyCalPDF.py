...
dp_rsc = abspath(join(__file__, pardir, pardir, "rsc"))
...
class pdfCalendar():
    ...
    def __init__(self,config_file,folderpath="./",CreateTempFile=False,
                 dpo=None, preserveAspectRatio=False,**kwargs):
        """ the creator
        Parameters:
        -----------
        config_file: dict
        ...
        dpo: str
            output folder for the generated pdf
        Returns:
        --------
        None - but creates a pdf file (if successful) under folderpath/out
        """
        root_folder = abspath(join(__file__,pardir,pardir,pardir))
        self.debugPath = abspath(join(root_folder,"out","log.txt"))
        self.logger = logging.getLogger("pyCalPDF")
        

        logging.debug(f"debug path {self.debugPath}")
        if dpo is None:
            self.PDFOutFolder = abspath(join(folderpath,"out"))
        else:
            self.PDFOutFolder = dpo
#        self.SetConfiguration(config_file,CreateTempFile)
        ...
        if "paths" in self.conf:
            ...
            if isabs(self.conf["paths"]):
                self.defaultpath = self.conf["paths"]
            else:
    ...
    def debug(self,TrueFalse,debugPath="./log.txt",debug_level=0,ShowPix = False):
        ...
        fileHandler = logging.FileHandler(self.debugPath)
        fileHandler.setFormatter(logFormatter)
        self.logger.addHandler(fileHandler)
        
        self.logger.debug(f"debug path {self.debugPath}")
...
#        log.close()
    def _log(self,title,value,level=0):
        """line = "**"+title+"\n"
        for el in list:
            if len(str(el))<1000:
                line = line + "\t"+str(el)
...
#        self.pcal.setFont(Fonts["FontType"], 30) 
                ...
                if len(content)>0:
                    ...
                    if ical.upper().find("MOON")>0 or ical.upper().find("LUNE")> 0:
                        ...
                        if content.lower().find("first quarter")>=0 or content.find("Premier quartier")>0:
                            ressource =abspath(join(dp_rsc, "img", "svg", "FirstQuarter.svg"))
                        elif content.lower().find("full moon")>=0 or content.find("Pleine lune")>0:
                            ressource =abspath(join(dp_rsc, "img", "svg", "FullMoon.svg"))
                        elif content.lower().find("last quarter")>=0 or content.find("Dernier quartier")>0:
                            ressource =abspath(join(dp_rsc, "img", "svg", "LastQuarter.svg"))

                        elif content.lower().find("new moon")>=0 or content.find("Nouvelle lune")>0:
                            ressource =abspath(join(dp_rsc, "img", "svg", "NewMoon.svg"))
                        svg_full_path = ressource
                    else:
                    ...
                    if drawing is None:
                        ...
                        # drawing = svg2rlg(abspath(join(self.defaultpath,"rsc","img","svg","Warning.svg"))) 
                        ressource =abspath(join(dp_rsc, "img", "svg", "Warning.svg"))
                    if not showContent:
                        content = " "
                    if header=="banner":
    ...
    def addMonth(self,MonthStart,NumberMonths,
        ...
        Modifies:
        ---------
            self.pcal - adds month picture (if ShowPix=True), month table and previous months
        """ 
        ...
        if self.template.find("no-pix")<0:
            ...
            if self.ShowPix:
                ...
                self.pcal.drawImage(image_full_path,x0,y0+height_mo+self.margins["bottom"],\
                    self.width-2*self.margins["right"],localheight-height_mo-2*self.margins["top"],\
        ...
        #            self.pcal.drawCentredString(0, 0,  MonthPictureIndex["description"])
                ...
                self.pcal.drawCentredString(self.width / 2,\
                     y0+height_mo+(localheight-height_mo-2*self.margins["top"])/2,\
...
#            dates = sorted(mycal.flat_events)
            #after - from 0.6.1y4'
            self._log("CY calendar dates",dates)
    ...
    def Generate(self):
        ...
        day = 1 #int(dtstart[6:8])
        print("WARNING on 2021/1/2 line 809 was changed from int(dtstart[6:8]) to 1")
        ...
        # calStart is first day of current month to be rendered on
        calStart = datetime.date(year,MonthStart,day) # type: datetime.date
        ...
        while (calStart <= dtend):
            if "pictures" in self.conf:
                picture_path = self.conf["pictures"][index % (len(self.conf["pictures"]))]
            else:
                #no picture path need to be defined in the conf file when in topbound nopix template
                picture_path = None
            self.addMonth(calStart,NumberMonths = MonthEnd-MonthStart+1,\
             CalType = self.conf["calscale"],\
